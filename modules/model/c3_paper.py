...
class C3(nn.Module):
    ...
    def forward(self,
        ...
        pos = (S * w * V.T).sum(dim=-1)  # (B,)
        neg = (S * w * (1. - V.T)).sum(dim=-1)  # (B,)

        # obtaining mean and standard deviation
        mean = pos / ((S * w).sum(dim=-1) + EPS)  # (B,)
    ...
    def infer_batch(self,
        ...
        total = pos + neg  # (B,)
        pos = eta * pos / (total + EPS)  # (B,)
        neg = eta * neg / (total + EPS)  # (B,)
        ...
        for e in pbar:
            ...
            with torch.no_grad():
                for X_query, y_query in val_dl:
                    ...
                    if loss.item() < best_loss:  # TODO: incorrect (but results on paper use this version)
                        best_loss = loss.item()
                        best_state_dict = self.state_dict()
